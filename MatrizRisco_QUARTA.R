...
#'*Script de automatização da Matriz de Risco Covid-19*
...
pacman::p_load(data.table, tidyverse, lubridate, tibbletime, dplyr, zoo, EpiEstim, formattable, DT, Hmisc, knitr)

# BANCOS UTILIZADOS -------------------------------------------------------
setwd('C:/Users/Tamara Dias/Desktop/Cievs/Produto 2 - Matriz de Risco Covid/Hoje')

valoresMatriz <- fread('valoresMatrizRisco.csv', sep=';')

painelhoje <- fread("Último painel do dia.csv",
                    select = c("id", "RA", "dataPrimeiroSintomas", "classificacaoFinal", "dataObito"))

hoje <- Sys.Date()
casosAtivos7dias <- read.csv2(paste0(format(hoje-7, '%d-%m-%y'),"casosAtivos7dias.csv"))

# Casos de 7 dias atrás leitura de pdf
texto_bruto1 <- pdf_text("01.pdf") %>%
    strsplit("\n") %>%
    unlist() %>%
    enframe(name = NULL, value = "Linha")
texto_bruto2 <- pdf_text("02.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto3 <- pdf_text("03.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto4 <- pdf_text("04.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto5 <- pdf_text("05.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto6 <- pdf_text("06.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto7 <- pdf_text("07.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")
texto_bruto8 <- pdf_text("08.pdf") %>%
  strsplit("\n") %>%
  unlist() %>%
  enframe(name = NULL, value = "Linha")


#  Pegando os valores de interesse dos pdfs -------------------------------
# Dia 1 - mais antigo
linha20.1 <- texto_bruto1[20,]
linha20.1 <- linha20.1 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.1 <- gsub(" ", ",", linha20.1, fixed = TRUE)
linha20.1 <- strsplit(as.character(linha20.1), ',')
linha20.1 <- data.frame(Linha=unlist(linha20.1))
linha20.1 <- as.numeric(linha20.1$Linha)
linha57.1 <- texto_bruto1[57,]
linha57.1 <- linha57.1 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.1 <- gsub(" ", ",", linha57.1, fixed = TRUE)
linha57.1 <- strsplit(as.character(linha57.1), ',')
linha57.1 <- data.frame(Linha=unlist(linha57.1))
linha57.1 <- as.numeric(linha57.1$Linha)

# Dia 2 
linha20.2 <- texto_bruto2[20,]
linha20.2 <- linha20.2 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.2 <- gsub(" ", ",", linha20.2, fixed = TRUE)
linha20.2 <- strsplit(as.character(linha20.2), ',')
linha20.2 <- data.frame(Linha=unlist(linha20.2))
linha20.2 <- as.numeric(linha20.2$Linha)
linha57.2 <- texto_bruto2[57,]
linha57.2 <- linha57.2 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.2 <- gsub(" ", ",", linha57.2, fixed = TRUE)
linha57.2 <- strsplit(as.character(linha57.2), ',')
linha57.2 <- data.frame(Linha=unlist(linha57.2))
linha57.2 <- as.numeric(linha57.2$Linha)

# Dia 3 
linha20.3 <- texto_bruto3[20,]
linha20.3 <- linha20.3 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.3 <- gsub(" ", ",", linha20.3, fixed = TRUE)
linha20.3 <- strsplit(as.character(linha20.3), ',')
linha20.3 <- data.frame(Linha=unlist(linha20.3))
linha20.3 <- as.numeric(linha20.3$Linha)
linha57.3 <- texto_bruto3[57,]
linha57.3 <- linha57.3 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.3 <- gsub(" ", ",", linha57.3, fixed = TRUE)
linha57.3 <- strsplit(as.character(linha57.3), ',')
linha57.3 <- data.frame(Linha=unlist(linha57.3))
linha57.3 <- as.numeric(linha57.3$Linha)

# Dia 4 
linha20.4 <- texto_bruto4[20,]
linha20.4 <- linha20.4 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.4 <- gsub(" ", ",", linha20.4, fixed = TRUE)
linha20.4 <- strsplit(as.character(linha20.4), ',')
linha20.4 <- data.frame(Linha=unlist(linha20.4))
linha20.4 <- as.numeric(linha20.4$Linha)
linha57.4 <- texto_bruto4[57,]
linha57.4 <- linha57.4 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.4 <- gsub(" ", ",", linha57.4, fixed = TRUE)
linha57.4 <- strsplit(as.character(linha57.4), ',')
linha57.4 <- data.frame(Linha=unlist(linha57.4))
linha57.4 <- as.numeric(linha57.4$Linha)

# Dia 5 
linha20.5 <- texto_bruto5[20,]
linha20.5 <- linha20.5 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.5 <- gsub(" ", ",", linha20.5, fixed = TRUE)
linha20.5 <- strsplit(as.character(linha20.5), ',')
linha20.5 <- data.frame(Linha=unlist(linha20.5))
linha20.5 <- as.numeric(linha20.5$Linha)
linha57.5 <- texto_bruto8[57,]
linha57.5 <- linha57.5 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.5 <- gsub(" ", ",", linha57.5, fixed = TRUE)
linha57.5 <- strsplit(as.character(linha57.5), ',')
linha57.5 <- data.frame(Linha=unlist(linha57.5))
linha57.5 <- as.numeric(linha57.5$Linha)

# Dia 6 
linha20.6 <- texto_bruto6[20,]
linha20.6 <- linha20.6 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.6 <- gsub(" ", ",", linha20.6, fixed = TRUE)
linha20.6 <- strsplit(as.character(linha20.6), ',')
linha20.6 <- data.frame(Linha=unlist(linha20.6))
linha20.6 <- as.numeric(linha20.6$Linha)
linha57.6 <- texto_bruto6[57,]
linha57.6 <- linha57.6 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.6 <- gsub(" ", ",", linha57.6, fixed = TRUE)
linha57.6 <- strsplit(as.character(linha57.6), ',')
linha57.6 <- data.frame(Linha=unlist(linha57.6))
linha57.6 <- as.numeric(linha57.6$Linha)

# Dia 7
linha20.7 <- texto_bruto7[20,]
linha20.7 <- linha20.7 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.7 <- gsub(" ", ",", linha20.7, fixed = TRUE)
linha20.7 <- strsplit(as.character(linha20.7), ',')
linha20.7 <- data.frame(Linha=unlist(linha20.7))
linha20.7 <- as.numeric(linha20.7$Linha)
linha57.7 <- texto_bruto7[57,]
linha57.7 <- linha57.7 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.7 <- gsub(" ", ",", linha57.7, fixed = TRUE)
linha57.7 <- strsplit(as.character(linha57.7), ',')
linha57.7 <- data.frame(Linha=unlist(linha57.7))
linha57.7 <- as.numeric(linha57.7$Linha)

# Dia 8 - dia atual
linha20.8 <- texto_bruto8[20,]
linha20.8 <- linha20.8 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha20.8 <- gsub(" ", ",", linha20.8, fixed = TRUE)
linha20.8 <- strsplit(as.character(linha20.8), ',')
linha20.8 <- data.frame(Linha=unlist(linha20.8))
linha20.8 <- as.numeric(linha20.8$Linha)
linha57.8 <- texto_bruto8[57,]
linha57.8 <- linha57.8 %>%
  rowwise() %>%
  mutate_all(funs(str_squish(.))) %>%
  ungroup()
linha57.8 <- gsub(" ", ",", linha57.8, fixed = TRUE)
linha57.8 <- strsplit(as.character(linha57.8), ',')
linha57.8 <- data.frame(Linha=unlist(linha57.8))
linha57.8 <- as.numeric(linha57.8$Linha)


# TAXAS DE OCUPAÇÃO ------------------------------------------------------
#Leitos
UTI_Publica_Direc <- sum(linha57.8[4],linha20.8[9])
UTI_Publica_Ocup <- sum(linha57.8[5],linha20.8[10])
UTI_Publica_Bloq <- sum(linha57.8[6],linha20.8[11])
UTI_Publica_Total <- sum(linha57.8[7],linha20.8[12])
...
UCI_Publica_Ocup <- valoresMatriz[5,2]
UCI_Publica_Bloq <- valoresMatriz[6,2]
UCI_Publica_Direc <- valoresMatriz[7,2]
UCI_Publica_Total <- valoresMatriz[8,2]
ENF_Publica_Ocup <- valoresMatriz[9,2]
ENF_Publica_Bloq <- valoresMatriz[10,2]
ENF_Publica_Direc <- valoresMatriz[11,2]
ENF_Publica_Total <- valoresMatriz[12,2]

# ESGOTAMENTO -------------------------------------------------------------
#Leitos ocupados por dia 

dia1 <- sum(linha20.1[9:11], linha57.1[4:6])
dia2 <- sum(linha20.2[9:11], linha57.2[4:6])
dia3 <- sum(linha20.3[9:11], linha57.3[4:6])
dia4 <- sum(linha20.4[9:11], linha57.4[4:6])
dia5 <- sum(linha20.5[9:11], linha57.5[4:6])
dia6 <- sum(linha20.6[9:11], linha57.6[4:6])
dia7 <- sum(linha20.7[9:11], linha57.7[4:6])
...
ativos_central7 <- casosAtivos7dias[1,1]
ativos_centroSul7 <- casosAtivos7dias[1,2]
ativos_leste7 <- casosAtivos7dias[1,3]
ativos_norte7 <- casosAtivos7dias[1,4]
ativos_oeste7 <- casosAtivos7dias[1,5]
ativos_sudoeste7 <- casosAtivos7dias[1,6]
ativos_sul7 <- casosAtivos7dias[1,7]
ativos_DF7 <- casosAtivos7dias[1,8]
...
tabela1 <- datatable(riscoDF,  width = 300,  caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ("Classificação de risco do DF")), 
                     options = list(lengthChange = FALSE, dom = 't', columnDefs = list(list(className = 'dt-center', targets = '_all')),
                                    autoWidth=T, columnDefs = list(list(width = '70px', targets = c(0,1))))) %>% 
...
webshot(html1, paste0(hoje," Tabela1 - Classificacao DF.png"), vwidth = 400,  vheight = 100, zoom = 6)
...
tabela2 <- datatable(tabelaRisco, width = 500, rownames = FALSE, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ("Indicadores que compõem a classificação de risco do DF")), 
...
webshot(html2, paste0(hoje," Tabela2 - Indicadores.png"), vwidth = 600,  vheight = 500, zoom = 8)

# > Matriz - DF -----------------------------------------------------------

# * Eixo 1 - Capacidade de Atendimento --------------------------------------
tabelaE1 <- matrix(c('Taxa de ocupação de Leitos UTI COVID-19',paste0(leitos_UTI,'%'), pontosI1, 
                     'Taxa de ocupação de Leitos Clínicos COVID-19',paste0(leitos_UTI_clinicos,'%'), pontosI2,
                     'Previsão de Esgotamento de Leitos UTI',dias_esg, pontosI3), ncol=3, byrow = T)
colnames(tabelaE1) <- c('Indicadores', 'Resultado', 'Pontos')

tabela3 <- datatable(tabelaE1, width = 500, rownames = FALSE, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ("Eixo 1 - Capacidade de Atendimento das Regiões de Saúde e Distrito Federal")), 
                     options = list(lengthChange = FALSE, dom = 't',                                                                                                                                                    
                                    columnDefs = list(list(className = 'dt-center', targets = 1:2)), pageLength = 3,
                                    columnDefs = list(list(width = '100px', targets = c(0,1,3))))) %>% 
  formatStyle(colnames(tabelaE1)[2],colnames(tabelaE1)[3],
              backgroundColor = styleEqual(c(0,1:9,10:18,19:30,31:40), c('yellowgreen',rep('yellow', 9),rep('orange', 9), rep('orangered',12) ,rep('mediumpurple',10))))
tabela3

html3 <- "tabela3.html"
saveWidget(tabela3, html3)
webshot(html3, paste0(hoje," Tabela3 - Eixo1.png"), vwidth = 600,  vheight = 500, zoom = 8)

# * Eixo 2 - Cenário Epidemiológico ---------------------------------------
tabelaE2 <- matrix(c('Variação do N° de Óbitos por COVID-19 nos últimos 14 dias',paste0(var_total_obt,'%'), pontosI4,
                     'Variação do N° de casos por COVID-19 nos últimos 14 dias',paste0(var_total,'%'), pontosI5,
                     'Atividade',atividade_DF, pontosI6), ncol=3, byrow = T)

colnames(tabelaE2) <- c('Indicadores', 'Resultado', 'Pontos')

tabela4 <- datatable(tabelaE2,  width = 500, rownames = FALSE, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ('Eixo 2 - Cenário Epidemiológico')), 
                     options = list(lengthChange = FALSE, dom = 't',
                                    columnDefs = list(list(className = 'dt-center', targets = 1:2)), pageLength = 3,
                                    columnDefs = list(list(width = '100px', targets = c(0,1,3))))) %>%
  formatStyle(colnames(tabelaE2)[2],colnames(tabelaE2)[3],
              backgroundColor = styleEqual(c(0,1:9,10:18,19:30,31:40), c('yellowgreen',rep('yellow', 9),rep('orange', 9), rep('orangered',12) ,rep('mediumpurple',10))))
tabela4

html4 <- "tabela4.html"
saveWidget(tabela4, html4)
webshot(html4, paste0(hoje," Tabela4 - Eixo2 - Cenario Epid.png"), vwidth = 600,  vheight = 500, zoom = 8)
...
sketch = htmltools::withTags(table(class = 'display', thead(tr(th(rowspan = 2, 'Região de Saúde'),th(colspan = 2, 'Variação do N° de Óbitos por COVID-19 nos últimos 14 dias')),tr(lapply(rep(c('Resultado', 'Pontos'), 1), th)))))
...
tabela5 <- datatable(tabelaE2RS[ , c(1, 2:4)], width = 500, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ('Eixo 2 - Cenário Epidemiológico')), container = sketch, rownames = FALSE, 
                     options = list(lengthChange = FALSE, dom = 't',
                                    columnDefs = list(list(className = 'dt-center', targets = 1:2)), pageLength = 8,
                                    columnDefs = list(list(targets = 3, visible = FALSE)),
                                    columnDefs = list(list(width = '80px', targets = c(0,1,3))))) %>%
...
webshot(html5, paste0(hoje," Tabela5 - Eixo2 - Var obitos.png"), vwidth = 600,  vheight = 500, zoom = 8)

# * Eixo 2 - Cenário Epidemiológico - Variação de casos 21 dias -----------
sketch = htmltools::withTags(table(class = 'display', thead(tr(th(rowspan = 2, 'Região de Saúde'),th(colspan = 2, 'Variação do N° de casos por COVID-19 nos últimos 21 dias')),tr(lapply(rep(c('Resultado', 'Pontos'), 1), th)))))
...
tabela6 <- datatable(tabelaE2RSCasos[ , c(1, 2:4)], width = 500, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ('Eixo 2 - Cenário Epidemiológico')), container = sketch, rownames = FALSE,
                     options = list(lengthChange = FALSE, dom = 't',
                                    columnDefs = list(list(className = 'dt-center', targets = 1:2)),
                                    pageLength = 8,
                                    columnDefs = list(list(targets = 3, visible = FALSE)),
                                    columnDefs = list(list(width = '70px', targets = c(0,1,3))))) %>% 
...
webshot(html6, paste0(hoje," Tabela6 - Eixo2 - Var casos.png"), vwidth = 600,  vheight = 500, zoom = 8)
...
tabela7 <- datatable(tabelaE2RSAtv[ , c(1, 2:4)], width = 500, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ('Eixo 2 - Cenário Epidemiológico')), container = sketch, rownames = FALSE, 
                     options = list(lengthChange = FALSE, dom = 't',
                                    columnDefs = list(list(className = 'dt-center', targets = 1:2)),
                                    pageLength = 8,
                                    columnDefs = list(list(targets = 3, visible = FALSE)),
...
html7 <- "tabela7.html"
saveWidget(tabela7, html7)
webshot(html7, paste0(hoje," Tabela7 - Eixo2 - Atividade.png"), vwidth = 600,  vheight = 500, zoom = 8)
...
tabela8 <- datatable(tabelaE2RSRisco[ , c(1, 2:3)], width = 500, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;', ('Eixo 2 - Cenário Epidemiológico')), 
                     ...
                     options = list(columnDefs = list(list(className = 'dt-center', targets = 1:2)), autoWidth=T, columnDefs = list(list(width = '90px', targets = c(0,1,2))),
                                    lengthChange = FALSE, dom = 't',                                                                                                                                                                                                                    pageLength = 8)) %>% 
  formatStyle(colnames(tabelaE2RSRisco)[3],colnames(tabelaE2RSRisco)[2],
              backgroundColor = styleEqual(c(0,1:9,10:18,19:30,31:40), c('yellowgreen',rep('yellow', 9),rep('orange', 9), rep('orangered',12) ,rep('mediumpurple',10))))
tabela8

html8 <- "tabela8.html"
saveWidget(tabela8, html8)
webshot(html8, paste0(hoje, " Tabela8 - Eixo2 - Grau de Risco.png"), vwidth = 600,  vheight = 500, zoom = 8)
